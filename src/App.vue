				...
				<div slot="start" style="height: 100%;">
					<sl-card class="card-preview">
						<div id="colorsHeader">
							...
							<sl-button size="medium" @click="darkMode()">Switch to Dark Mode</sl-button>
						</div>
						<div>
							<div
							...
							>
								...
								<div class="colors">
									...
									<div
										...
										:key="number"
										:style="{backgroundColor:`var(${color.variable + '-' + number})`}"
									></div>
								</div>
							</div>
						</div>
	...
	export default {
		...
		mounted() {
			this.colors.forEach((color, i) => {
				this.colors[i].value = getComputedStyle(document.documentElement).getPropertyValue(
					`${color.variable}-500`
				);
			});
		},
